...
async def _process_resume_inner(ctx: Dict[str, Any], document_id: str):
    ...
    chunks = chunk_text(raw_text, target_tokens=300, overlap_tokens=50)
    ...
    prompt_path = pathlib.Path("prompts/resume/extract_v1.md")
    if prompt_path.exists():
        system_prompt = prompt_path.read_text(encoding="utf-8")
        system_prompt = system_prompt.replace("{document_text}", raw_text).replace("{filename}", doc.storage_path.split("/")[-1])
    else:
        system_prompt = f"Extract resume.\n\nUNTRUSTED_DOCUMENT\n{raw_text}"
    ...
    async def extract_with_retry():
        try:
            resp = await router.route("reasoning", route_ctx, "structured", messages, schema=ExtractedResumeProfile)
            return resp.result
        except Exception as e: # Validation Error or other
            logger.warning(f"Initial extraction failed: {e}. Retrying with corrective instruction.")
            retry_messages = messages + [
                LLMMessage(role="assistant", content="<malformed_output_or_error>"),
                LLMMessage(role="user", content="your previous output did not match the required schema; return only valid JSON matching it exactly")
            ]
            resp = await router.route("reasoning", route_ctx, "structured", retry_messages, schema=ExtractedResumeProfile)
            return resp.result

    try:
        parsed: ExtractedResumeProfile = await extract_with_retry()
        
        async with db.begin() as conn:
            for proj in parsed.projects:
                await conn.execute(text("""
                    INSERT INTO candidate_projects (candidate_id, title, role, summary)
                    VALUES (:cid, :title, :role, :summary)
                """), {"cid": doc.candidate_id, "title": proj.title[:100], "role": proj.role[:100], "summary": proj.summary})
                
            for skill in parsed.skills:
                await conn.execute(text("""
                    INSERT INTO candidate_skills (candidate_id, name, proficiency)
                    VALUES (:cid, :name, :prof)
                """), {"cid": doc.candidate_id, "name": skill.name[:50], "prof": skill.proficiency})
                
            for exp in parsed.experiences:
                await conn.execute(text("""
                    INSERT INTO candidate_experiences (candidate_id, company, title, start_date, end_date, description)
                    VALUES (:cid, :company, :title, :start_date, :end_date, :description)
                """), {"cid": doc.candidate_id, "company": exp.company, "title": exp.title, "start_date": exp.start_date, "end_date": exp.end_date, "description": exp.description})

            for edu in parsed.education:
                await conn.execute(text("""
                    INSERT INTO candidate_education (candidate_id, institution, degree, field_of_study, start_date, end_date)
                    VALUES (:cid, :inst, :deg, :field, :start_date, :end_date)
                """), {"cid": doc.candidate_id, "inst": edu.institution, "deg": edu.degree, "field": edu.field_of_study, "start_date": edu.start_date, "end_date": edu.end_date})
        ...
        logger.error(f"Extraction failed after retry: {e}")
        async with db.begin() as conn:
            await conn.execute(text("UPDATE documents SET processing_status = 'failed', error_message = :err WHERE id = :id"), {"id": document_id, "err": f"Extraction failed: {str(e)}"})
...
async def _analyze_job_inner(ctx: Dict[str, Any], job_id: str):
    ...
    prompt_path = pathlib.Path("prompts/jd/extract_v1.md")
    if prompt_path.exists():
        system_prompt = prompt_path.read_text(encoding="utf-8")
        system_prompt = system_prompt.replace("{document_text}", job.description or "")
    else:
        system_prompt = f"Extract JD.\n\nUNTRUSTED_DOCUMENT\n{job.description or ''}"
    ...
    async def extract_with_retry():
        try:
            resp = await router.route("reasoning", route_ctx, "structured", messages, schema=ExtractedJobBlueprint)
            return resp.result
        except Exception as e:
            logger.warning(f"Initial JD extraction failed: {e}. Retrying.")
            retry_messages = messages + [
                LLMMessage(role="assistant", content="<malformed_output_or_error>"),
                LLMMessage(role="user", content="your previous output did not match the required schema; return only valid JSON matching it exactly")
            ]
            resp = await router.route("reasoning", route_ctx, "structured", retry_messages, schema=ExtractedJobBlueprint)
            return resp.result

    try:
        parsed: ExtractedJobBlueprint = await extract_with_retry()
        
        async with db.begin() as conn:
            for req in parsed.job_requirements:
                await conn.execute(text("""
                    INSERT INTO job_requirements (job_id, category, description, importance)
                    VALUES (:jid, :cat, :desc, :imp)
                """), {"jid": job_id, "cat": req.category, "desc": req.skill_text, "imp": req.priority})
                
            for topic in parsed.likely_topics:
                await conn.execute(text("""
                    INSERT INTO job_blueprints (job_id, dimension, target_signal, scoring_rubric, created_by)
                    VALUES (:jid, 'behavioral', :sig, '{"pass": "good", "fail": "bad"}', :cid)
                """), {"jid": job_id, "sig": topic.topic + " - " + topic.rationale, "cid": job.created_by})
